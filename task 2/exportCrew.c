...
// Step1Result parallel_step1(int A[], int B[], int *N, int *r, int *s) {
//     int primeSize = *N - 1;
//     int *Aprime = malloc(primeSize * sizeof(int));
//     int *Bprime = malloc(primeSize * sizeof(int));
//     Step1Result result = {Aprime, Bprime, primeSize};

//     // Simulación de paralelismo
//     #pragma omp parallel for
//     for (int i = 1; i < *N; i++) {
//         int indexa = i * ceil((*r) / (double)(*N));
//         int indexb = i * ceil((*s) / (double)(*N));
//         Aprime[i - 1] = A[indexa - 1];
//         Bprime[i - 1] = B[indexb - 1];
//     }
//     return result;
...
// Step2Element* parallel_step2(Step1Result *obj, int *N) {
//     Step2Element *V = malloc((obj->primeSize * 2) * sizeof(Step2Element));

//     #pragma omp parallel for
//     for (int i = 0; i < *N - 1; i++) {
//         int index = binarySearch(obj->Bprime, *N, obj->Aprime[i], '>');
//         int vIndex = i + (index >= 0 ? index : *N);
//         Step2Element temp = {obj->Aprime[i], i + 1, 'A'};
//         V[vIndex] = temp;
//     }
...
//     for (int i = 0; i < *N - 1; i++) {
//         int index = binarySearch(obj->Aprime, *N, obj->Bprime[i], '>');
//         int vIndex = i + (index >= 0 ? index : *N - 1);
//         Step2Element temp = {obj->Bprime[i], i + 1, 'B'};
//         V[vIndex] = temp;
//     }
//     return V;
...
// Step3Element* parallel_step3(Step2Element *V, int *A, int *B, int r, int s, int N) {
//     Step3Element *Q = malloc(N * sizeof(Step3Element));
//     Q[0] = (Step3Element){1, 1};
...
//             int index = binarySearch(B, s, V[vIndex - 1].num, '>');
//             if (index < 0) index = s;
//             Q[i - 1] = (Step3Element){V[vIndex - 1].indx * ceil(r / (double)N), index + 1};
//         } else {
//             int index = binarySearch(A, r, V[vIndex - 1].num, '>');
//             if (index < 0) index = r;
//             Q[i - 1] = (Step3Element){index + 1, V[vIndex - 1].indx * ceil(s / (double)N)};
//         }
//     }
//     return Q;
// }

// int* parallel_sort(Step3Element* Q, Step2Element* V, int* A, int* B, int r, int s, int N) {
//     int* C = malloc((r + s) * sizeof(int));
...
//         while (x <= r && (i == N || A[x - 1] < V[vIndex].num)) {
//             if (y > s || A[x - 1] <= B[y - 1]) {
//                 C[x + y - 2] = A[x - 1];
//                 x++;
//             } else {
...
//         while (y <= s && (i == N || B[y - 1] < V[vIndex].num)) {
//             C[x + y - 2] = B[y - 1];
//             y++;
//         }
...
// int* parallel_crew_Merge(int A[], int B[], int r, int s, int N) {
//     Step1Result primes = parallel_step1(A, B, &N, &r, &s);
//     Step2Element* V = parallel_step2(&primes, &N);
//     Step3Element* Q = parallel_step3(V, A, B, r, s, N);
//     int* C = sort(Q, V, A, B, r, s, N);
//     return C;
// }
